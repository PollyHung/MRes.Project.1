...
save.rds <- "~/MRes_project_1/docs/SC_SEQ/ova/raw/lambrechtslab/ova.rds"
...
write.table(sub_metadata[, c("barcode", "newCelliD")], 
            file = paste0(inferCNV.dir, "total_cellAnnotations.txt"), 
            sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

## reference group names
cancers_id = sub_metadata$newCelliD[grepl(pattern="*_Cancer", sub_metadata$newCelliD)] %>% unique
ref_group_names <- setdiff(unique(sub_metadata$newCelliD), cancers_id)

## file 5: define output directory for this sample 
output_dir <- "~/MRes_project_1/docs/SC_SEQ/ova/raw/lambrechtslab/InferCNV/72_hr_runtime/"

## inferCNV object 
infercnv_obj <- CreateInfercnvObject(raw_counts_matrix = sub_matrix, 
                                     annotations_file = paste0(inferCNV.dir, "total_cellAnnotations.txt"), 
