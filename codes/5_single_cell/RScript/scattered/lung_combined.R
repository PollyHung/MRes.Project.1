...
setwd("~/MRes_project_1/docs/SC_SEQ/lung/processed/combined/plots")
...
LAM <- Read10X(data.dir = "~/MRes_project_1/docs/SC_SEQ/lung/raw/lambrechtslab/count_matrix/")
LAM <-  CreateSeuratObject(counts = LAM, min.cells = 3, min.features = 200)

BSC <- merge(GSE, y=LAM, project="lung")

saveRDS(BSC, "~/MRes_project_1/docs/SC_SEQ/lung/processed/combined/lung.rds")
...
BSC <- RunHarmony(object = BSC,
                  group.by.vars = c("company", "sample.type", "sample"),
                  assay.use = "RNA", 
