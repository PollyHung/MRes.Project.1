...
hh_lung.dir <- "~/MRes_project_1/docs/HH_lung/facets/seg_cval_50/_hisens_diplogR.adjusted.seg"
cbioportal_mel.dir <- "~/MRes_project_1/docs/cBioPortal_melanoma/cBioPortal_melanoma.seg"
cbioportal_lung.dir <- "~/MRes_project_1/GISTIC2/cbioportal/lung/segmentation.seg"
...
## Section 2.3: HH_lung ----------------------------------------------------------------------------
segmentation <- read.table(hh_lung.dir)
colnames(segmentation) <- c("ID", "Chromosome", "Start", "End", "Num_Probes", "Segment_Mean")
segmentation_list <- split(segmentation, segmentation$ID)
...
for(i in names(segmentation_list)){ 
  ...
  RunAbsolute("x", sigma.p = 0, max.sigma.h = 0.015, min.ploidy = 0.95, 
              max.ploidy = 10, primary.disease = "ov", 
              platform = "Illumina_WES", sample.name = i, 
              results.dir = "~/MRes_project_1/docs/absolute/hh_lung/", 
              max.as.seg.count = 1500, max.non.clonal = 0.05, max.neg.genome = 0.005, 
              copy_num_type = "total", maf.fn = NULL, min.mut.af = NULL, output.fn.base = NULL, 
              verbose = FALSE)
  
  setwd("~/MRes_project_1/docs/absolute/hh_lung/")
  load(paste0(i, ".ABSOLUTE.RData"))
  write.csv(seg.dat$segtab, file = paste0(i, ".csv"))
...
## Section 2.4: cbioportal_mel ----------------------------------------------------------------------------
segmentation <- read.table(cbioportal_mel.dir, header = TRUE)
colnames(segmentation) <- c("ID", "Chromosome", "Start", "End", "Num_Probes", "Segment_Mean")
segmentation_list <- split(segmentation, segmentation$ID)
...
for(i in names(segmentation_list)){ 
  ...
  RunAbsolute("x", sigma.p = 0, max.sigma.h = 0.015, min.ploidy = 0.95, 
              max.ploidy = 10, primary.disease = "ov", 
              platform = "Illumina_WES", sample.name = i, 
              results.dir = "~/MRes_project_1/docs/absolute/cbioportal_mel/", 
              max.as.seg.count = 1500, max.non.clonal = 0.05, max.neg.genome = 0.005, 
              copy_num_type = "total", maf.fn = NULL, min.mut.af = NULL, output.fn.base = NULL, 
              verbose = FALSE)
  
  setwd("~/MRes_project_1/docs/absolute/cbioportal_mel/")
  load(paste0(i, ".ABSOLUTE.RData"))
  write.csv(seg.dat$segtab, file = paste0(i, ".csv"))
...
## Section 2.5: cbioportal_lung ----------------------------------------------------------------------------
segmentation <- read.table(cbioportal_lung.dir, header = TRUE)
colnames(segmentation) <- c("ID", "Chromosome", "Start", "End", "Num_Probes", "Segment_Mean")
segmentation_list <- split(segmentation, segmentation$ID)
...
for(i in names(segmentation_list)){ 
  ...
  RunAbsolute("x", sigma.p = 0, max.sigma.h = 0.015, min.ploidy = 0.95, 
              max.ploidy = 10, primary.disease = "ov", 
              platform = "Illumina_WES", sample.name = i, 
              results.dir = "~/MRes_project_1/docs/absolute/cbioportal_lung/", 
              max.as.seg.count = 1500, max.non.clonal = 0.05, max.neg.genome = 0.005, 
              copy_num_type = "total", maf.fn = NULL, min.mut.af = NULL, output.fn.base = NULL, 
              verbose = FALSE)
  
  setwd("~/MRes_project_1/docs/absolute/cbioportal_lung/")
  load(paste0(i, ".ABSOLUTE.RData"))
  write.csv(seg.dat$segtab, file = paste0(i, ".csv"))
