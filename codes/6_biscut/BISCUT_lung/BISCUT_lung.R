## Control panel -------- 
getwd()
setwd("/rds/general/user/ph323/home/MRes_project_1/BISCUT/BISCUT_lung")
...
tel_background_file <- 'docs/PANCAN_lung_tels_200605.txt'
cent_background_file <- 'docs/PANCAN_lung_cents_200605.txt'
...
## load data ------- 
lung <- read.table("docs/TCGA_lung", header = TRUE)
PANCAN <- read.table("docs/PANCAN_ISAR.seg.txt", header = TRUE)

## data inspection 
length(unique(lung$sample)) ## TCGA 1021 lung patients 
length(unique(PANCAN$Sample)) ## 10872 PANCAN samples 
length(intersect(lung$sample, PANCAN$Sample)) ## there are 989 intersecting patients 

## which PANCAN samples == lung? 
select_sample <- intersect(lung$sample, PANCAN$Sample)

## filter and write file to the HPC space 
PANCAN_lung <- filter(PANCAN, PANCAN$Sample %in% select_sample)
if(!"PANCAN_lung_ISAR.seg.txt" %in% list.files("docs")){ ## if file not in docs directory 
  write.table(PANCAN_lung, file = "docs/PANCAN_lung_ISAR.seg.txt", sep = "\t", 
...
## save the image 
save.image(file = "/rds/general/user/ph323/home/MRes_project_1/BISCUT/BISCUT_lung/BISCUT.RData")
if(LOAD){
  load("/rds/general/user/ph323/home/MRes_project_1/BISCUT/BISCUT_lung/BISCUT.RData")
}
