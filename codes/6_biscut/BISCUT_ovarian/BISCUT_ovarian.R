## Control panel -------- 
getwd()
setwd("/rds/general/user/ph323/home/MRes_project_1/codes/7_biscut/BISCUT_ovarian/")
...
tel_background_file <- 'docs/PANCAN_ova_tels_200605.txt'
cent_background_file <- 'docs/PANCAN_ova_cents_200605.txt'
...
## load data ------- 
ovarian <- readRDS("docs/seg_tcga_cnv.rds")
PANCAN <- read.table("PANCAN_ISAR.seg.txt", header = TRUE)

## data inspection 
length(unique(ovarian$sample)) ## TCGA 606 ovarian patients 
length(unique(PANCAN$Sample)) ## 10872 PANCAN samples 
length(intersect(ovarian$sample, PANCAN$Sample)) ## there are 589 intersecting patients 

## which PANCAN samples == ovarian? 
select_sample <- intersect(ovarian$sample, PANCAN$Sample)

## filter and write file to the HPC space 
PANCAN_ova <- filter(PANCAN, PANCAN$Sample %in% select_sample)
if(!"PANCAN_ova_ISAR.seg.txt" %in% list.files("docs")){ ## if file not in docs directory 
  write.table(PANCAN_ova, file = "docs/PANCAN_ova_ISAR.seg.txt", sep = "\t", 
              row.names = FALSE, quote = FALSE, col.names = TRUE)
}

## Step 1 BISCUT_preprocessing.py: -------------------------------
## Run BISCUT_preprocessing.py on that data to generate breakpoint_files_*date* directory 
## which contains the breakpoints for telomeric and centromeric copy number alterations across all 
## chromosome arms. 
reticulate::use_python("~/anaconda3/envs/CIEnv/bin/python3", required = TRUE)
source_python('BISCUT_preprocessing.py')

## Step 2 plot_BISCUT_results.R: -------------------------------
...
write.table(newthing,file=paste(resultsfolder,paste("KS_pvalues_",thedate,'_',toString(ci),".txt",sep=''),sep='/'),
            sep='\t',row.names=FALSE, quote = FALSE, col.names = TRUE)

## save the image 
save.image(file = "/rds/general/user/ph323/home/MRes_project_1/BISCUT/BISCUT_ovarian/BISCUT.RData")
if(LOAD){
  load("/rds/general/user/ph323/home/MRes_project_1/BISCUT/BISCUT_ovarian/BISCUT.RData")
}

## Step 5 combine_BISCUT_results.py --------------------
#call the function to combine BISCUT results and process some files for plotting
reticulate::use_python("~/anaconda3/envs/CIEnv/bin/python3", required = TRUE)
source_python('combine_BISCUT_results.py')
selectedgenes = read.table("genes.txt") %>% unlist
